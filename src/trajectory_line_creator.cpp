...
namespace trajectory_generator {
    ...
    TrajectoryLineCreator::TrajectoryLineCreator(ros::NodeHandle nh, ros::NodeHandle pnh) :
            pnh_(pnh),
            visual_detection_sub_(),
            reconfigure_server_(),
            initial_config_(),
            initial_received_(false),
            obstacleTrustThreshold_(0.5)
    {
        trajectory_pub_ = nh.advertise<geometry_msgs::PoseArray>("trajectory_out", 1);
        debugTrajectory_pub_ = nh.advertise<visualization_msgs::MarkerArray>("trajectory_markers_out", 1);
    ...
    bool TrajectoryLineCreator::init() {
        ...
        visual_detection_sub_ = pnh_.subscribe("costmap_in", 100, &TrajectoryLineCreator::visual_detection_callback, this);

        if (!pnh_.getParam("obstacleTrustThreshold", obstacleTrustThreshold_)) {
          ROS_ERROR("Parameter 'obstacleThrustThreshold' not found, shutting down!");
          return false;
        }
        return true;
    }

    void TrajectoryLineCreator::visual_detection_callback(const drive_ros_msgs::RoadLaneConstPtr& road_in) {
      ROS_INFO("Road lane callback received");
...
//        float distanceToObstacle = 0;
        ...
        // todo: we do not have this yet as we do not use the laserscanner for mapping obstacles
